...
class CGAN(object):
    def __init__(self, args):
        ...
        self.data_loader = load_data(self.dataset, args.imbalance, self.batch_size, True, False)
    ...
    def visualize_results(self, epoch, fix=True):
        ...
        if fix:
            ...
            self.sample_y_ = torch.zeros(self.batch_size, self.class_num).scatter_(1, torch.randint(0, self.class_num - 1, (self.batch_size, 1)).type(torch.LongTensor), 1)
        ...
        if self.use_fake_data:
            return (samples, torch.zeros(self.sample_num)*self.minority)
