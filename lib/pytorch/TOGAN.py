...
class TOGAN(object):
    def __init__(self, args):
        ...
        self.data_loader = load_data(self.dataset, args.imbalance, self.batch_size, True, False)      # imbalanceed dataset
        ...
        self.C = classifier(input_dim=data.shape[1], output_dim=self.class_num, input_size=self.input_size, class_num=self.class_num)
        self.G_optimizer = optim.Adam(self.G.parameters(), lr=args.lrG, betas=(args.beta1, args.beta2))
        self.D_optimizer = optim.Adam(self.D.parameters(), lr=args.lrD, betas=(args.beta1, args.beta2))
        self.C_optimizer = optim.Adam(self.C.parameters(), lr=args.lrC, betas=(args.beta1, args.beta2))
        ...
        for i in range(self.class_num-1):
            self.sample_z_[i*(self.class_num-1)] = torch.rand(1, self.z_dim)
            for j in range(1, self.class_num-1):
                self.sample_z_[i*(self.class_num-1) + j] = self.sample_z_[i*(self.class_num-1)]
        ...
        if self.use_fake_data:
            ...
            for i in range(self.class_num-1):
                self.temp_y[i*(self.class_num-1): (i+1)*(self.class_num-1)] = temp
    ...
    def train(self):
        ...
        for epoch in range(self.epoch):
            ...
            for iter, (x_, y_) in enumerate(self.data_loader):
                ...
                C_ = self.C(x_)
                C_ = C_.unsqueeze(2).unsqueeze(3).expand(self.batch_size, self.class_num, self.input_size, self.input_size)
                D_C_ = self.D(x_, C_)
                ...
                if ((iter + 1) % 100) == 0:
                    print("Epoch: [%2d] [%4d/%4d] D_loss: %.8f, G_loss: %.8f, C_loss: %.8f" %
                          ((epoch + 1), (iter + 1), self.data_loader.dataset.__len__() // self.batch_size, D_loss.item(), G_loss.item(), C_loss.item()))
    ...
    def visualize_results(self, epoch, fix=True):
        ...
        if fix:
            ...
            """ random noise """
            self.sample_y_ = torch.zeros(self.batch_size, self.class_num).scatter_(1, torch.randint(0, self.class_num - 2, (self.batch_size, 1)).type(torch.LongTensor), 1)
            i = 0
            while i < self.batch_size:
                if self.sample_y_[i][self.minority] == 1:
                    self.sample_y_[i][-1] = 1
                i += 1
        ...
        if self.use_fake_data:
            return (samples, torch.ones(self.sample_num) * self.minority)
